...
final[is.na(final)] <- 0
write.csv(final, 
          file=paste0(n_path,'splitMetrics/test/WetlandType.csv'),
          row.names = FALSE)
```
...
final[is.na(final)] <- 0
write.csv(final,
          file=paste0(n_path,'/splitMetrics/test/WetlandFreq.csv'),
          row.names=FALSE)
```
...
final[is.na(final)] <- 0
write.csv(final, 
          file=paste0(n_path,'/splitMetrics/test/WetlandMag.csv'),
          row.names = FALSE)
```
...
final[is.na(final)] <- 0
write.csv(final, 
          file = paste0(n_path,'/splitMetrics/test/WetlandMagAvg.csv'),
          row.names=FALSE)
```
...
write.csv(final, 
          file=paste0(n_path,'/splitMetrics/test/WetlandDist.csv'),
          row.names=FALSE)
```
...
for (nm in tables){
  ...
  zonal[is.na(zonal)] <- 0
  up_nm <- capwords(nm)
  out <- paste0(hm_path,'splitMetrics/test/WetDrain',up_nm,'.csv')
  write.csv(zonal, 
            file=out,
            row.names=FALSE)
}
