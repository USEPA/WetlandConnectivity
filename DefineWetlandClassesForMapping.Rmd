...
wc$Class_NoImp[!(wc$Class_NoImp %in% keep)] = 'Other'
...
### Make 30-m rasters for focal regions 
...
print '------------' + year + '------------'
wd = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/MapRasters/Wetlands/'
wd2 = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/ExampleLocations/'
wd3 = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/FinalTables/'

inTemplate = wd2 + 'pipestem_template_2001.tif'
...
### Make summaries of wetland classes at Tiner/NARS regions for mapping (by area)
...
for(i in 1:2){
  ...
  for(k in 1:length(cols2)){
    ...
    if(k == 1){
      ...
      wc4 = merge(wc4, tmp, by = 'RasterID', all=T)
    }
  }
  ...
  for(k in 1:length(cols)){
    ...
    wc4[cols[k]] = apply(wc4[c(x)], 1,  which.max)
    wc4[cols[k]] = x2[wc4[cols[k]][[1]]]
  }
...
#   }
#   
#   x2 = sub('Class_NoImp', '',x)
#   out_tmp$Class_NoImp_bycount = apply(out_tmp[c(cols2)], 1,  which.max)
#   out_tmp$Class_NoImp_bycount = x2[out_tmp$Class_NoImp_bycount]
#   out_tmp = out_tmp[ , c('RasterID','Class_NoImp_bycount')]
  
#  wc4 = merge(wc4, out_tmp, by = 'RasterID')  
  
  write.csv(wc4, paste0(out_path, '/WetClasses_region_byarea_summaries.csv'), row.names=F)
}
...
### Make summaries of wetland classes at Tiner/NARS regions for mapping (by count)
...
#Loop through all and non-riparian wetlands
for(i in 1:2){
  print(i)
  if(i == 1){
    wc3 = wc2
    out_path = all_path #All
  }else{
    wc3 = wc2[wc2$TypeRip == 0,] #Non-riparian
    out_path = nrp_path
  }
  
  #Convert codes from single column to binary multi-columns
  for(k in 1:length(cols)){    
    codes = na.omit(unique(wc3[cols[k]]))[[1]]
    codes = codes[order(codes)]
    for(j in 1:length(codes)){
  ...
  #Group by and grab columns to process
  wcgroup = group_by(wc3, RasterID)   
  cols2 = names(wc3[14:length(wc3)])
  
  #Create sums of each binary column
  for(k in 1:length(cols2)){
    print(cols2[k])
    cmd_txt = paste0('summarize(wcgroup, ', cols2[k], ' = sum(', cols2[k], ', na.rm=T))')
    #print(cmd_txt)
  ...
  #Find column with max val for each class and place that code into row location
  for(k in 1:length(cols)){
    print(cols[k])
    x = names(wc4)[grep(cols[k], names(wc4))] 
    x2 = sub(cols[k], '',x)
  ...
  write.csv(wc4, paste0(out_path, '/WetClasses_region_bycount_summaries.csv'), row.names=F)
}
...
### Make raster of Tiner/NARS regional summaries
...
#wd2 = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/AllWetlands/'
wd3 = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/FinalTables/TinerNARSTables/'
...
inCSV = wd3 + tmp + 'WetClasses_region_summaries.csv'
inTable = pd.read_csv(inCSV)
cols = inTable.columns.values[2:]
#inTable['WET_ID'] = inTable['WetId'].str.split('_').str[0]

for col in cols:
    print col
    out_dir = wd + tmp
    outRas = out_dir + col + '_900m.tif'
    if not arcpy.Exists(outRas):
        catcsv2raster2(inTable, col, inTemplate, outRas, dtype='Float', idName='RasterID')
