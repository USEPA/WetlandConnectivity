...
### Get flow type for wetlands with no path - I commented out, we no longer use, right?  They're all classified as riparian now I believe - MHW
 ```{r, eval=F}
# library(raster)
# library(dplyr)
# r = c('VALUE_11', 'VALUE_12', 'VALUE_21', 'VALUE_22')
...
### Make wetland basin flood freq table
...
for(i in 1:length(rpus)){
  cat_dir = accum_cat
  print(rpus[i])
  flood_freq = read.csv(paste0(accum_cat, 'FloodFreq_MEAN_', rpus[i], '.csv'))   
  flood_freq$FLOOD_FREQ = (flood_freq$SUM / flood_freq$COUNT)
  summary(flood_freq)
  flood_freq$FLOOD_CAT <- cut(flood_freq$FLOOD_FREQ, breaks=c(-1600,-1200, -852, -505, -157, 200), include.lowest=TRUE)
  table(flood_freq$FLOOD_CAT)
  levels(flood_freq$FLOOD_CAT) <- c('0','1','2','3','4')
  flood_freq <- flood_freq[c(1,5)]
  write.csv(flood_freq, paste0(final_path, 'FloodFreq_', rpus[i], '.csv'), row.names=F)  
}     
```
...
wetpoints_path = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/AllWetlandPoints')
...
for (i in 1:21){
  ...
  for (k in 1:length(vpu_rpus)){
    ...
    GridFeat_lookup = read.dbf(paste0(nhd_path,"/NHDPlus",rgns[i],"/NHDPlus",hydro.rgns[i],"/NHDPlusCatchment/featureidgridcode.dbf"))
    wetID_CatID$COMID = GridFeat_lookup$FEATUREID[match(wetID_CatID$cat,GridFeat_lookup$GRIDCODE)]
    wetID_CatID = wetID_CatID[c('COMID')]
    WetID_COMID = cbind(wetpoints@data, wetID_CatID)
    WetID_COMID = WetID_COMID[c('POINTID','COMID')]
...
eco9 = read.csv('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/eco9_all.csv')

for(k in 1:nrow(rpus)){
  print(rpu)
  lookup = read.csv(paste0(out_lookup, 'WetlandsID_COMID_', rpus[i], '.csv'))
  lookup$WSA_9 = eco9$WSA_9[match(lookup$COMID, eco9$COMID)]
  write.csv(lookup, paste0(out_lookup, 'WetlandsID_COMID_', rpus[i], '.csv'), row.names=F)  
}
```

# Add COMID, Eco to final tables
FlowType
```{r, eval=F}
accum_path = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/WetlandPath/Accumulation/')
...
final_lookup = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/WetlandPath/LookupTables/')
