...
lookup_tables = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/LookupTables/'
...
for(i in 1:length(rpus)){
  print(rpus[i])
  elevmax = read.csv(paste0(accum_path, 'elev_cm_max_', rpus[i], '.csv'))
  elevmin = read.csv(paste0(accum_path, 'elev_cm_min_', rpus[i], '.csv'))  
  rise = merge(elevmax, elevmin)
  rise$rise = (rise$MAX - rise$MIN) / 100 #convert to meters
  rise$rise = rise$rise + 0.1
  rise = rise[ ,c('COMID', 'rise')]
...
for(i in 1:length(rpus)){
  print(rpus[i])
  flmax = read.csv(paste0(accum_path, 'fldown_max_', rpus[i], '.csv'))
  flmin = read.csv(paste0(accum_path, 'fldown_min_', rpus[i], '.csv'))  
...
for (i in 1:59){
  ...
  nlcd$Total <- rowSums(nlcd[,2:17])
  
  #calculate %s for each nlcd category
  for (k in 2:17){
    nlcd[,k] = 100.0 * nlcd[,k]/nlcd[,18]
  } 
  nlcd = nlcd[c(1:17)]
...
preciplist = variables[grep("zonalstats_US2yr24ha_mm_10x_", variables)]
...
for(i in 1:length(rpus)){
  ...
  porosity = read.csv(paste0(accum_path, 'AvgPorosity_L1L2__max_', rpus[i], '.csv'))
  names(porosity) <- c('PATHID', 'porosity_max')
  perm = read.csv(paste0(accum_path, 'AvgSoilPerm_mm_d_83__min_', rpus[i], '.csv'))
...
for(i in 1:length(rpus)){
  ...
  print(rpus[i])
  flow_cls = read.csv(paste0(accum_path, 'BdrckPerm_4_classes_Alb83__sum_', rpus[i], '.csv'))   
  missing <- r[is.na(match(r, names(flow_cls)))]
  flow_cls[,paste(missing)] <- c(0)
  
  flow_cls = flow_cls[, c('COMID', r)] #make in right order
