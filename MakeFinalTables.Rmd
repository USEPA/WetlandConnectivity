...
```{r, eval=F}
year = '2011'
final_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/FinalTables/WetlandTables/')
...
accum_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPath/Accumulation/')
accum_cat = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandCat/Accumulation/')
# precip_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/WetlandCat/Accumulation/'
lookup_tables = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPath/LookupTables/')
...
for(i in 1:length(rpus)){
  ...
  run$run = (run$MAX - run$MIN) + 15 # give non-isolated wetlands a minimum distance of 15m
...
```{r, eval=F}
library(stringr)
#path_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/WetlandPath/Accumulation'
path_dir = accum_path
...
# Mannings (path)
library(foreign)
if (year=='2011') rat <- read.dbf('L:/Priv/CORFiles/Geospatial_Library_Projects/StreamCat/LandscapeRasters/QAComplete/nlcd2011.tif.vat.dbf')
...
mannings = read.csv('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/old_LookupTables/NLCD_Mannings_Lookup.csv')
head(mannings)
...
### Combine to make start of shallow table (path)
```{r, eval=F}
for(i in 1:length(rpus)){
  print(rpus[i])
  run = read.csv(paste0(accum_path, 'run_m_', rpus[i], '.csv'))
...
for(i in 1:length(rpus)){
  ...
  flow_freq = read.csv(paste0(accum_path, 'DrainFreq_MIN_', rpus[i], '.csv'))
  missing <- r[is.na(match(r, names(flow_cls)))]
  flow_cls[,paste(missing)] <- c(0)
  ...
  flow_cls$FlowFreq <- flow_freq$MIN[match(flow_cls$PATHID, flow_freq$PathID)]
  print(summary(flow_cls))
...
```{r, eval=F}
lookup_dir = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPath/LookupTables')
...
### We dropped this and no longer use!
...
#   runoff_freq <- runoff_freq[c('BasinID','RUNOFF_FREQ','RUNOFF_CAT')]
#   runoff_freq <- runoff_freq[c('BasinID','RUNOFF_FREQ')]
#   print(summary(runoff_freq))
#   write.csv(runoff_freq, paste0(final_path, 'SatOvr2yr24hmm_', rpus[i], '.csv'), row.names=F)  
# }     
```
...
wetpoints_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPoints')
lookup_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPath/LookupTables/')
rpu_lookup_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'1/WetlandPath/LookupTables/')
out_lookup = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/LookupTables/')
...
for (i in 1:21){
  ...
  for (k in 1:length(vpu_rpus)){
    ...
    wetID_CatID = cbind(wetpoints@data, wetID_CatID)
    wetID_CatID = wetID_CatID[c('GRID_CODE','COMID')]
    names(wetID_CatID)[1] = 'WetID'
...
Update 7/10/17 - Scott wants individual Tiner polygons identified so need to
use UnID code in the Tine shapefile rather than Tiner wetland region name
```{r, eval=F}
library(rgdal); library(rgeos)
...
lookup_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandPath/LookupTables/')
comeco_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/LookupTables/')
...
for(i in 1:length(rpus)){
  ...
  in_table$Riparian[in_table$Riparian==1] = 100
  in_table$DomFlow = as.character(in_table$DomFlow)
  in_table$DomFlow[in_table$Riparian==100] = 'Riparian'
...
for(i in 1:length(rpus)){
  ...
  names(in_table)[6] = 'DomDrainClass'
  names(in_table)[7] = 'DrainFreq'
  in_table = in_table[ , c('WET_ID','PATHID','COMID','CatAreaSqKm','WSA_9','WET_ID_RPU', 'STATE','WetlandRegion', 'Riparian','DomDrainClass','DrainFreq')]
  in_table$Riparian[in_table$Riparian==1] = 100
  print(summary(in_table))
...
for(i in 1:length(rpus)){
  ...
  in_table = in_table[ , c('WET_ID','PATHID','COMID','WSA_9','WET_ID_RPU', 'STATE','WetlandRegion', 'Riparian','CanalInfluence')]
  in_table$Riparian[in_table$Riparian==1] = 100
...
library(foreign)
table_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/FinalTables/WetlandTables/')
wet_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/AllWetlands_rpu/')
...
for (k in 1:length(tables)){
  ...
  saveRDS(result, file = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/FinalTables/',tables[k],'.rds'))
...
#DrainClass_
tmp = readRDS(paste0(rds_path, 'DrainClass_.rds'))[c('WET_ID', 'DomDrainClass','DrainFreq')]
nrow(tmp); length(unique(tmp$WET_ID))
names(tmp) = c('WetId','DomFreqClassPa','FreqPa')
combined = merge(combined, tmp, by = 'WetId')
...
combined = combined[c('WetId','PathID','COMID','WSA_9','WetRegion',
                        ...
                        'DomFreqClassPa','TypeSh','TypeShDp','Type','MagOv','MagSh','FreqPa',
...
library(stringr)
wetland_zonal_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/WetlandOnlyZonal')
table_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/FinalTables/WetlandTables/')
...
rds_path = paste0('L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'b/FinalTables/')
fullwetlands <- readRDS(paste0(rds_path, 'WetConnectMetrics_',year,'.rds'))
table(fullwetlands$FreqPa, fullwetlands$TypeRip)
# If wetlands are riparian, frequency should be 1 - High
fullwetlands$FreqPa[fullwetlands$TypeRip==100] = 1
...
fullwetlands$Type <- ifelse(fullwetlands$Type=="Riparian", "Ripar", ifelse(fullwetlands$Type=="Overland", "NRSur", "NRSub"))
table(fullwetlands$Type, fullwetlands$Type_Full)
fullwetlands$Mag <- ifelse(fullwetlands$Type=="Ripar", "VF",
  ifelse(fullwetlands$Type=="NRSur", ifelse(fullwetlands$MagOv<=24, "VF", ifelse(fullwetlands$MagOv/24<=14, "FA", "MO")),
  ifelse(fullwetlands$MagSh<=1, "VF", ifelse(fullwetlands$MagSh<=14, "FA", ifelse(fullwetlands$MagSh/365.25<=1, "MO",
  ifelse(fullwetlands$MagSh/365.25<=10, "SL", "VS"))))))
fullwetlands$Freq <- ifelse(fullwetlands$FreqPa==0, "L", ifelse(fullwetlands$FreqPa==0.5, "M", "H"))
fullwetlands$Imp <- ifelse(fullwetlands$Type=="Ripar", ifelse(rowSums(fullwetlands[,c("ImpDrImperv","ImpDrAg")])==0, "N",
  ifelse(fullwetlands[,"ImpDrImperv"]<=5 & fullwetlands[,"ImpDrAg"]<=5, "L", "H")),
  ifelse(rowSums(fullwetlands[,c("ImpDrImperv","ImpDrAg","ImpPaAg","ImpPaLev","ImpPaCan")])==0, "N",
  ifelse(fullwetlands[,"ImpDrImperv"]<=5 & fullwetlands[,"ImpDrAg"]<=5 & fullwetlands[,"ImpPaAg"]<=5 & fullwetlands[,"ImpPaLev"]==0 & fullwetlands[,"ImpPaCan"]==0, "L", "H")))
fullwetlands$Class <- ifelse(is.na(fullwetlands[, "Type"]) | is.na(fullwetlands[, "Mag"]) |is.na(fullwetlands[, "Freq"]) | is.na(fullwetlands[, "Imp"]), NA,
  paste0(fullwetlands[, "Type"], fullwetlands[, "Mag"], fullwetlands[, "Freq"], fullwetlands[, "Imp"]))
fullwetlands <- fullwetlands[c(1:4,31,5:14,32,16:30,15,33:36)]
saveRDS(fullwetlands, file = paste0(rds_path, 'WetConnectMetrics_',year,'.rds'))
write.csv(fullwetlands, file = paste0(rds_path, 'WetConnectMetrics_',year,'.csv'), row.names=F) 
...
```{r, eval=F}
library(dplyr)
old_lookup = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/old_LookupTables/'
lookup_tables = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/LookupTables_final/'
master_table = read.csv(paste0(old_lookup,'eco9_all.csv'))
wet_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/FinalTables/WetlandTables_final/'
cat_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/FinalTables/NHDPlusCatchmentTables/'
...
```{r, eval=F}
accum_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/Accumulation/'
...
library(foreign)
wet_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/FinalTables/WetlandTables_final/'
wetland_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/AllWetlands_rpu/'
accum_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/Accumulation/'
final_lookup = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/LookupTables_final/'
cat_path = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/FinalTables/NHDPlusCatchmentTables/'
master_table = read.csv(paste0(old_lookup,'eco9_all.csv'))
files = list.files(accum_path, pattern = 'NLCD'); rpus = c()
for(i in 1:length(files)){
...
saveRDS(tmp, file = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/FinalTables/ForScott/comid_gridcode.rds')
