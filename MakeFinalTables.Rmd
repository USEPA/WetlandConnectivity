...
for(i in 1:length(rpus)){
  ...
  # Set row where all NA to zero
  flow_cls[is.na(flow_cls)] <- 0
  flow_cls$DomClass = type_names[apply(flow_cls[, 2:5], 1, which.max)]
  # which.max assigns first column to rows where all zero (path fel outside raster boundary)
  # we'll recycle missing and set DomFlow to NA for these records
  missing <- flow_cls[apply(flow_cls[, 2:5], MARGIN = 1, function(x) all(x == 0)), ]
...
for(i in 1:length(rpus)){
  ...
  in_table = in_table[ , c('WET_ID','PATHID','COMID','CatAreaSqKm','WSA_9','WET_ID_RPU', 'STATE','WetlandRegion', 'Riparian','DomDrainClass')]
  in_table$Riparian[in_table$Riparian==1] = 100
  print(summary(in_table))
...
Wetland_regions = Wetland_regions[c('Name')]
...
#   }
  ...
  wetpoints$Name = as.character(wetpoints$Name)
  wetpoints$Name[is.na(wetpoints$Name)] = 'None'  
  
  lookup = read.csv(paste0(out_lookup, 'WetlandsID_COMID_', rpus[i], '.csv'))
  #lookup$STATE = wetpoints$STUSPS10[match(lookup$WetID, wetpoints$GRID_CODE)]
  lookup$WetlandRegion = wetpoints$Name[match(lookup$WetID, wetpoints$GRID_CODE)]
  write.csv(lookup, paste0(out_lookup, 'WetlandsID_COMID_', rpus[i], '.csv'), row.names=F)
}
```
...
# comeco_path = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/LookupTables/')
# 
# tables = c('AgDrainage_Basin_', 'AgDrainage_Path_', 'BasinAreaSqKm_', 'CanalInfluence_', 'DrainClass_', 'FloodFreq_', 
#            'FlowClass_', 'Imperviousness_', 'LeveeInfluence_', 'OverlandFlow_', 'SatOvr2yr24hmm_', 'ShallowFlow_', 'WetBasinArea_')
# 
...
tables = list.files(table_path, pattern = "01")
...
for(i in 1:length(rpus)){
  print(rpus[i])
  wetlands = read.dbf(paste0(wet_path, 'Wetlands_', rpus[i], '.tif.vat.dbf'))
  in_table = read.csv(paste0(table_path, 'BasinAreaSqKm_', rpus[i], '.csv'))
  ...
  #names(in_table) = c('WET_ID', 'PATHID', 'COMID', 'WSA_9', 'STATE', 'WetlandRegion', 'Riparian', 'BasinAreaSqKm', 'WetlandAreaSqKm', 'CatAreaSqKm')
  write.csv(in_table, paste0(table_path, 'BasinAreaSqKm_', rpus[i], '.csv'), row.names=F)
}
...
tables = c('AgDrainage_Basin_', 'AgDrainage_Path_', 'BasinAreaSqKm_', 'CanalInfluence_', 'DrainClass_',
           'FlowClass_', 'Imperviousness_', 'LeveeInfluence_', 'OverlandFlow_', 'SatOvr2yr24hmm_', 'ShallowFlow_')
...
tiner = tiner[c('WetId_rpu','WetlandRegion')]
names(tiner) = c('WetId', 'WetRegion')



#AgDrainage_Basin_
combined = readRDS(paste0(rds_path, 'AgDrainage_Basin_.rds'))
combined = subset(combined, select = -c(WET_ID, WetlandRegion))
head(combined)
names(combined) = c('PathID', 'COMID', 'CatAreaSqKm', 'WSA_9', 'WetId', 'State', 'TypeRip', 'ImpDrAg')

#Tiner Wetland Region
combined = merge(combined, tiner, by = 'WetId')

#AgDrainage_Path_
tmp = readRDS(paste0(rds_path, 'AgDrainage_Path_.rds'))[c('WET_ID_RPU', 'PctAgDrainPath')]
names(tmp) = c('WetId','ImpPaAg')
combined = merge(combined, tmp, by = 'WetId')

#BasinAreaSqKm_
tmp = readRDS(paste0(rds_path, 'BasinAreaSqKm_.rds'))[c('WET_ID_RPU', 'BasinAreaSqKm', 'WetlandAreaSqKm')]
names(tmp) = c('WetId','DrainAreaSqKm','WetlAreaSqKm')
combined = merge(combined, tmp, by = 'WetId')

#CanalInfluence_
tmp = readRDS(paste0(rds_path, 'CanalInfluence_.rds'))[c('WET_ID_RPU', 'CanalInfluence')]
names(tmp) = c('WetId','ImpPaCan')
combined = merge(combined, tmp, by = 'WetId')

#DrainClass_
tmp = readRDS(paste0(rds_path, 'DrainClass_.rds'))[c('WET_ID_RPU', 'DomDrainClass')]
names(tmp) = c('WetId','FreqSh')
combined = merge(combined, tmp, by = 'WetId')

#FlowClass_
tmp = readRDS(paste0(rds_path, 'FlowClass_.rds'))[c('WET_ID_RPU', 'Shallow','ShallowDeep','Overland','DomFlow')]
names(tmp) = c('WetId', 'TypeSh','TypeShDp','TypeOv','Type')
combined = merge(combined, tmp, by = 'WetId')

#Imperviousness_
tmp = readRDS(paste0(rds_path, 'Imperviousness_.rds'))[c('WET_ID_RPU', 'BasinImperv')]
names(tmp) = c('WetId', 'ImpDrImperv')
combined = merge(combined, tmp, by = 'WetId')

#LeveeInfluence_
tmp = readRDS(paste0(rds_path, 'LeveeInfluence_.rds'))[c('WET_ID_RPU', 'LeveeInfluence')]
names(tmp) = c('WetId', 'ImpPaLev')
combined = merge(combined, tmp, by = 'WetId')

#OverlandFlow_
tmp = readRDS(paste0(rds_path, 'OverlandFlow_.rds'))[c('WET_ID_RPU', 'PRECIP_2YR24HR_MM', 'slope','Manning','run','t_overland')]
names(tmp) = c('WetId', 'Ppt2Yr24Hr', 'Slope','Manning','Run','MagOv')
combined = merge(combined, tmp, by = 'WetId')

#SatOvr2yr24hmm_
tmp = readRDS(paste0(rds_path, 'SatOvr2yr24hmm_.rds'))[c('WET_ID_RPU', 'SatOvr2yr24hmm')]
names(tmp) = c('WetId', 'FreqOv')
combined = merge(combined, tmp, by = 'WetId')

#ShallowFlow_
tmp = readRDS(paste0(rds_path, 'ShallowFlow_.rds'))[c('WET_ID_RPU', 'porosity_max','perm_min','t_shallow')]
names(tmp) = c('WetId', 'PorosityMax','PermMin','MagSh')
combined = merge(combined, tmp, by = 'WetId')

combined = combined[c('WetId','PathID','COMID','CatAreaSqKm','WSA_9','WetRegion',
                        'State','DrainAreaSqKm','WetlAreaSqKm','TypeRip','TypeOv',
                        'TypeSh','TypeShDp','Type','MagOv','MagSh','FreqOv','FreqSh',
