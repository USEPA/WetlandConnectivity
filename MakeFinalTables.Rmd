...
for(i in 1:length(rpus)){
  ...
  lookup$CanalInfuence = levee$SUM[match(lookup$PATHID,canals$PathID)]
  if (! "CanalInfuence" %in% colnames(lookup)) lookup$CanalInfuence = 0
  lookup$CanalInfuence[lookup$CanalInfuence > 0] = 1
  lookup$CanalInfuence[is.na(lookup$CanalInfuence)] = 0
  canals = lookup
  write.csv(canals, paste0(final_path, 'CanalInfuence_', rpus[i], '.csv'), row.names=F)
}
```
...
```{r, eval=F}
library(foreign)
centroid_path = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/AllWetlands_centroids/')
nhd_path = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/RESOURCE/PHYSICAL/HYDROLOGY/NHDPlusV21'
...
out_lookup = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/WetlandPath/LookupTables_final/')
