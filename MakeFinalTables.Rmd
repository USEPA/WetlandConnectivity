...
for (i in 1:21){
  ...
  for (k in 1:length(vpu_rpus)){
    ...
    if (any(is.na(WetID_COMID$COMID))){
      print(vpu_rpus[k])
      # get nearest Hydroregion where over didn't associate lake with region
      # missing <- lake_points[is.na(lake_points$EPA_Region),]
      # Regions = spTransform(Regions, CRS('+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs')) 
      # m = gDistance(missing, Regions, byid=TRUE) 
      # row = apply(m, 2, function(x) which(x==min(x))) 
      # Region_missing = Regions@data[row,]
      # Region_missing = Region_missing[c(2)]
      # names(Region_missing) = c('MissingID')
      # missing@data = cbind(missing@data, Region_missing)
      # missing$EPA_Region = missing$MissingID
    }
