...
for(i in 1:length(rpus)){
  ...
  in_table = in_table[ , c('PATHID','WET_ID','COMID','WSA_9', 'STATE','WetlandRegion','Shallow', 'ShallowDeep', 'Overland', 'Riparian', 'DomFlow')]
  summary(in_table)
...
for(i in 1:length(rpus)){
  ...
  in_table = in_table[ , c('PATHID','BASINID','COMID','WSA_9', 'STATE','WetlandRegion','PRECIP_2YR24HR_MM', 'slope', 'Manning', 'Riparian','run', 't_overland')]
...
for(i in 1:length(rpus)){
  ...
  allwet$WetlandRegion <- comeco$WetlandRegion[match(allwet$WET_ID, comeco$WetID)]
  in_table = merge(in_table, allwet, by.x='PATHID', by.y='STRMLNK_ID',all.y=T)  
  in_table = in_table[ , c('PATHID','WET_ID','COMID','WSA_9', 'STATE','WetlandRegion', 'run', 'slope', 'porosity_max', 'Riparian','perm_min', 't_shallow')]
  #in_table$run[is.na(in_table$PATHID)] = 0
  #in_table$slope[is.na(in_table$PATHID)] = 0
