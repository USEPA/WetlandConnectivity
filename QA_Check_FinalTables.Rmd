...
#Check that hole in base layer of runoff freq corresponds to NAs from ArcGIS for 10g
...
tmp = read.csv(paste0(wd, var, pattern, '.csv'))
var2 = 'BasinAreaSqKm_'
tmp2 = read.csv(paste0(wd, var2, pattern, '.csv'))
tmp = tmp[is.na(tmp$PctAgDrainBasin),'WET_ID']
tmp2 = tmp2[is.na(tmp2$AreaSqKm),'WET_ID']
setdiff(tmp, tmp2) #These end up being outside of ag drainage raster
...
for(i in 1:length(files)){
  ...
  wetlands = read.dbf(paste0(wet_path, 'Wetlands_', pattern, '.tif.vat.dbf'))
  print(paste0('wetlands = ', nrow(wetlands)))
  wetland_paths = read.dbf(paste0(cost_path, 'StreamLink_', pattern, '.tif.vat.dbf'))
