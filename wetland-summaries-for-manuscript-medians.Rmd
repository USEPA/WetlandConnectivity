...
wetsummary <- wet %>% group_by(Type) %>% summarise(WetAreaSqKm=sum(WetAreaSqKm), NumWetlands=n(), 
                                                   ...
                                                   iqr_travel_days=IQR(travel, na.rm = T))
...
wetsummary$iqr_da_to_wa[nrow(wetsummary)] <- IQR(wet$Drainarea_to_wetarea, na.rm = T)

wetsummary$median_pathLen_m[nrow(wetsummary)] <- median(wet$Run, na.rm = T)
wetsummary$iqr_pathLen_m[nrow(wetsummary)] <- IQR(wet$Run, na.rm = T)

wetsummary$median_travel_days[nrow(wetsummary)] <- median(wet$travel, na.rm = T)
wetsummary$iqr_travel_days[nrow(wetsummary)] <- IQR(wet$travel, na.rm = T)
...
tn$tmp <- tn$TNrate_filtered * tn$ha
