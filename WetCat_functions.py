...
def Accumulation(arr, COMIDs, lengths, upStream, tbl_type, ID_column, zonal_type='sum'):
    '''
    __author__ =  "Marc Weber <weber.marc@epa.gov>" 
                  "Ryan Hill <hill.ryan@epa.gov>"
    ...
    zonal_type            : default of 'mean' but can also change to 'max' or 'min' to get 
    '''
    zonal_type = str.upper(zonal_type)    
    coms = np.array(arr.iloc[:,0])  #Read in COMIDs
    ...
    for k in range(0,len(cols)):
        ...
        if 'PctFull' in col:
            ...
            x = 0
            for i in range(0, len(lengths)):
                if zonal_type=='SUM' or zonal_type=='MEAN' or zonal_type=='PERCENT':
                    z[i] = np.nansum(d[x:x + lengths[i]])
                if zonal_type=='MIN':
                    z[i] = np.nanmin(d[x:x+lengths[i]]) 
                if zonal_type=='MAX':
                    z[i] = np.nanmax(d[x:x+lengths[i]]) 
                x = x + lengths[i]
    ...
    if tbl_type == 'Ws':
        outDF.columns = np.append(ID_column, map(lambda x : x.replace('Cat', 'Ws'),cols.values))
    elif tbl_type == 'UpCat':
        outDF.columns = np.append(ID_column, 'Up' + cols.values)
    else: 
        outDF.columns = np.append(ID_column, map(lambda x : x.replace('Cat', str.capitalize(tbl_type)),cols.values))
