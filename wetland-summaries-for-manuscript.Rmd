...
* pctWetFiltered = % of CONUS filtered by wetlands (by wetland type)
* AreaFilteredSqKm = Area of CONUS filtered by wetlands (by wetland type)
...
rds_path <- 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011b/FinalTables/'
...
#Percent of US filtered by wetlands
...
outdf$AreaFilteredSqKm <- (outdf$CatCount * 900) / 1e6
outdf$pctWetFiltered <- (outdf$AreaFilteredSqKm / 7663941.7) * 100
outdf <- subset(outdf, select=c(Type, pctWetFiltered, AreaFilteredSqKm))
...
write.csv(wetsummary, 'L:/Public/rhill/bkup-nmodeling/figs-tables-summaries/summary_table.csv')
...
out_path <- 'D:/WorkFolder/WetlandConnectivity/Meetings/call-20190724/maps/'
...
totalN[paste0('pct', names(totalN)[4:ncol(totalN)])] <- (totalN[4:ncol(totalN)] / totalN$totalN) * 100

totalN$test <- rowSums(totalN[,4:7], na.rm = T)

totalN2 <- totalN[totalN$totalN > totalN$test, ]
...
head(tmp)
tmp$pctNFiltered <- rowSums(tmp[, 8:ncol(tmp)])
tmp$pctN_NoFilter <- 100 - tmp$pctNFiltered
tmp <- subset(tmp, select = -KgN_NoFilter)

write.csv(tmp, paste0(out_path, 'pct_filtered.csv'), row.names = F)


outsummary <- colSums(totalN2[, c(2, 4:7)])
outsummary$wetNKg <- sum(outsummary[2:length(outsummary)])
outsummary <- data.frame(outsummary)

outpct <- (outsummary[2:length(outsummary)] / outsummary$totalN) * 100
...
rds_path <- 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011b/FinalTables/'
cat_path <- 'D:/WorkFolder/WetlandConnectivity/WetConnect_Apr2019/Wetlands_NLCD2011b/WetlandCat/WetCats/'
...
wet$Magnitude <- ifelse(wet$Type == 'NRSur' | wet$Type == 'NRSubPd', wet$MagOv, wet$MagSh)
...
write.csv(wet, paste0(rds_path, 'WetlandConnectivity-ScienceHub-19092019.csv'), row.names = F)
