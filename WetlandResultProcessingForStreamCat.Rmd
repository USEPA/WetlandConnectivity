...
for (k in 1:length(tables)){
  ...
  X <- split(zonal, zonal$VPU)
  if (k!=5){
    lapply(1:length(X), function(i) write.csv(X[[i]]['COMID',paste0('Cat','CatDrainAreaSqKm',capwords(str_replace(tables[k],"_",""))),]  [c(1,3,2,4)], 
                                      file = paste0(zonal_dir, '/WetDrain',capwords(str_replace(tables[k],"_","")),'_', names(X[i]), ".csv"),
                                      row.names = FALSE))
  }
  if (k==5){
    lapply(1:length(X), function(i) write.csv(X[[i]]['COMID','CatSum'], 
                                      file = paste0(zonal_dir, '/WetDrain',capwords(str_replace(tables[k],"_","")),'_', names(X[i]), ".csv"),
                                      row.names = FALSE))
  }
}
...
# Below is the code for running the same as above on NRSA split-cat metrics
...
tables <- c('manure', 'fert', 'cbnf', 'CMAQ', 'PointN_')
...
for (nm in tables){
  ...
  # Issue of NAs with CMAQ data
  names(result)[1] <- 'WetId'
  if (nm == 'PointN_') result = result[c(1:2)]
  final <- merge(result, final, by='WetId', all.x=TRUE)
  final$State <- allwet$State[match(final$WetId, allwet$WetId)]
  final$COMID <- allwet$COMID[match(final$WetId, allwet$WetId)]
  if (nm != 'PointN_') {
  ...
  if (nm != 'PointN_'){
          ...
          by_COMID <- in_table %>%
            ...
            dplyr::summarize(CAT_COMID=unique(CAT_COMID), 
                             ...
                             CatDrainAreaSqKm = sum(DrainAreaSqKm),
                             CatLoad = CatMean * (CatDrainAreaSqKm * 100)
                             )
  } else {
          ...
          by_COMID <- in_table %>%
            group_by(SITE_ID) %>%
            dplyr::summarize(CAT_COMID=unique(CAT_COMID), 
                             CatSum= sum(SUM*DrainAreaSqKm) / sum(DrainAreaSqKm),
                             CatDrainAreaSqKm = sum(DrainAreaSqKm))
  }
  ...
  zonal = as.data.frame(by_COMID)
  nm <- capwords(nm)
  names(zonal)[3] <- paste0('Cat',str_replace(nm,"_",""))
  if (nm != 'PointN_'){names(zonal)[3] <- paste0('Cat', nm, 'Load')}
  zonal[is.na(zonal)] <- 0
  if (nm != 'PointN_'){zonal <- zonal[c(1,2,4,3,5)]} else{zonal <- zonal[c(1,2,4,3)]}
