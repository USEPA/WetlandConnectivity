...
for (k in 1:length(tables)){
  ...
  if (k == 1) {
    ...
    final <- merge(final, result, by='WetId', all.x=TRUE)
    }
  if (k > 1) {
    ...
    temp <- temp[c(1,3,7)]
    names(temp)[1] <- 'WetId'
    final <- merge(final, temp, all.x=TRUE)
    }
}
final$State <- allwet$State[match(final$WetId, allwet$WetId)]
final$COMID <- allwet$COMID[match(final$WetId, allwet$WetId)]
final$DrainAreaSqKm <- final$AREA * 1e-6
final <- final[c(1,8:9,7,3:6)]
final$SumNitrogen <- rowSums(final[,5:8])
saveRDS(final, file = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/FinalTables/NitrogenInputs.rds'))
