...
tables = c('manure_', 'fert_', 'cbnf_', 'CMAQ_')
...
for (k in 1:length(tables)){
  ...
  if (k == 1) {
    result <- ldply(paste0(table_path,cur_list), read.dbf)
    names(result)[7] <- tables[k]
    result <- result[c(1,3,7)]
    names(result)[1] <- 'WetId'
    }
  if (k > 1) {
    temp <- ldply(paste0(table_path,cur_list), read.dbf)
    names(temp)[7] <- tables[k]
    temp <- temp[c(1,3,7)]
    names(temp)[1] <- 'WetId'
    result <- merge(result, temp, all=TRUE)
    }
}
result$State <- allwet$State[match(result$WetId, allwet$WetId)]
result$COMID <- allwet$COMID[match(result$WetId, allwet$WetId)]
result$DrainAreaSqKm <- result$AREA * 1e-6
result <- result[c(1,4,3,5,2,6:8)]
result <- result[!is.na(result$CMAQ_),]
result$SumNitrogen <- rowSums(result[,5:8])
saveRDS(result, file = paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD',year,'/FinalTables/NitrogenInputs.rds'))
...
by_COMID <- in_table %>%
  group_by(COMID) %>%
  dplyr::summarize(CatSumNitrogen= mean(SumNitrogen), CatDrainAreaSqKm = sum(DrainAreaSqKm))
head(by_COMID)
zonal = as.data.frame(by_COMID)
...
lapply(1:length(X), function(i) write.csv(X[[i]][c(1,3,2)], 
                                      file = paste0(zonal_dir, '/WetlandNitrogen_', names(X[i]), ".csv"),
                                      row.names = FALSE))
```
