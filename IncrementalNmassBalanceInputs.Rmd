...
fullwetlands = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/FinalTables/WetConnectMetrics_2011.csv'
for(i in 1:length(rpus)){
  in_table <- read.csv(paste0(lookup_tables, 'AllWetlands_StreamLink_Lookup_',rpus[i],'.csv'))
  in_table <- in_table[c(1:3)]
  ...
  nfert <- read.dbf(paste0(table_path,'fert_',rpus[i],'.dbf'))
  names(nfert) <- toupper(names(nfert))
  nfert$CatFert <- nfert$SUM/nfert$COUNT
  nman <- read.dbf(paste0(table_path,'manure_',rpus[i],'.dbf'))
  names(nman) <- toupper(names(nman))
  nman$CatMan <- nman$SUM/nman$COUNT
  ncbnf <- read.dbf(paste0(table_path,'cbnf_',rpus[i],'.dbf'))
  names(ncbnf) <- toupper(names(ncbnf))
  ncbnf$CatCBNF <- ncbnf$SUM/ncbnf$COUNT
  ncmaq <- read.dbf(paste0(table_path,'CMAQ_',rpus[i],'.dbf'))
  names(ncmaq) <- toupper(names(ncmaq))
  ...
  final$WetAreaSqKm <- fullwetlands$WetAreaSqKm[match(final$WET_ID,fullwetlands$WetId)]
  final <- final[c(1,3,14,5:10,4,2,11:13)]
