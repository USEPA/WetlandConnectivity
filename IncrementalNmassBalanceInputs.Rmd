...
Freq = the connectivity frequency of that path (non-accumulated)
ttsub = travel time (sub-surface)
ttsurf = travel time (surface)
Conn_dom = is the path’s dominant connection surface or subsurface (will be used to select ttsub or ttsurf for calculation)
...
nman <- plyr::ldply(paste0(table_path,fert_list), read.csv)
nman$CatMan <- nman$SUM/nfert$COUNT
cbnf_list =variables[grep(tables[3],variables)]
ncbnf <- plyr::ldply(paste0(table_path,fert_list), read.csv)
ncbnf$CatCBNF <- ncbnf$SUM/nfert$COUNT
cmaq_list =variables[grep(tables[4],variables)]
ncmaq <- plyr::ldply(paste0(table_path,fert_list), read.csv)
ncmaq$CatCMAQ <- ncmaq$SUM/nfert$COUNT
for(i in 1:length(rpus)){
  ...
  ttsurf <- read.csv(paste0(results_path,'ShallowFlow_',rpus[i],'.csv'))
  ttsurf <- ttsurf[c(1,6)]
  final <- merge(ttsurf, in_table,by='PATHID', all=TRUE)
  ttsub <- read.csv(paste0(results_path,'OverlandFlow_',rpus[i],'.csv'))
  ttsub <- ttsub[c(1,7)]
  final$ttsub <- ttsub$t_overland[match(final$PATHID,ttsub$PATHID)]
  ...
  final <- final[c(1,3,5:8,4,2,9:10)]
