...
```{r, eval=F}
library(raster)
library(sf) #change to sf operation
library(rgdal)
library(stringr)
library(foreign)
require(dplyr)
...
year = '2011'
working_dir = 'D:/WorkFolder/WetlandConnectivity/WetConnect_Apr2019/Wetlands_NLCD'

path_dir = paste0(working_dir, year, 'b/WetlandPath/CostPaths')
points_dir = paste0(working_dir, year,'b/WetlandPoints')
accum_path = paste0(working_dir, year, 'b/WetlandPath/Accumulation/')
...
for (i in 1:length(path_list)){
  print(path_list[i])
  rpu = substr(path_list[i], 14, 16)
  outfile = paste0(path_dir, '/duplicated_', rpu,'.csv')
  if(!file.exists(outfile)){
    if(file.exists(paste0(path_dir,'/',path_list[i]))){
      streamlink_ras = raster(paste0(path_dir,'/',path_list[i]))
      #Need to create point from raster now?
      wetpoint = raster(paste0(points_dir, '/WetlandPoints', rpu, '.tif'))
      wetpoint = rasterToPoints(wetpoint)
      wetpoint = st_as_sf(data.frame(wetpoint), coords = c("x", "y"), crs = 5070)
      results = extract(streamlink_ras, wetpoint)
      
      tmp = results[duplicated(results)]
      tmp = unique(tmp)
      tmp = tmp[!is.na(tmp)]
