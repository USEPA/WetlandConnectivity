...
2. Use geopandas and groupby query to select just the point in each wetland region group with largest flow accumulation value(the wetland outlet)
```{r, engine='python', engine.path='C:/Python27/ArcGISx6410.4/python.exe', eval=F}
import arcpy
import os
from arcpy.sa import *
...
for region in inputs.keys():
    for hydro in inputs[region]:
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                if not arcpy.Exists(wetpoints_dir + "/WetlandPoints" + dirs[-3:]+ ".tif"):
                    print dirs
                    ...
                    lowerLeft = arcpy.Point(fac.extent.XMin,fac.extent.YMin)
                    cellSize = fac.meanCellWidth
                    #Convert rasters to numpy arrays
                    fac = arcpy.RasterToNumPyArray(fac)
                    wet = arcpy.RasterToNumPyArray(wetrpu_dir + "/WetlandsRgnGrp_" + dirs[-3:] + ".tif")
                    shp = wet.shape
                    mn = np.min(wet)
                    #Flatten arrays 
                    wetflat = wet.flatten()
                    ...
                    wetflat = ma.masked_values(wetflat, value=mn)
                    #Get unique values
                    unq = np.unique(wetflat)
                    #Find locations of max fac value within each wetland
                    locs = ndimage.maximum_position(fac, labels=wetflat, index=unq)
                    #Create empty numpy array that has the same
                    empty = np.full(fac.shape, mn, dtype='int32')
                    #Put wetland IDs into empty raster at max fac locations
                    empty.flat[locs] = unq
                    #Reshape vector to same as input raster
                    empty = empty.reshape(shp)
                    #Convert to esri raster and write out                 
                    newRaster = arcpy.NumPyArrayToRaster(empty, lowerLeft, cellSize, value_to_nodata=mn)
                    newRaster.save(wetpoints_dir + "/WetlandPoints" + dirs[-3:] + ".tif")  
```  
...
NHDDir = "L:/Priv/CORFiles/Geospatial_Library_Resource/PHYSICAL/HYDROLOGY/NHDPlusV21"
Year = '2001'
FullStreamsDir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + Year + '/FullStreams'
...
2. Run the Arc Cost Path tool using wetland outlet points, NHDPlus hydro DEM, and full streams null grid
```{r, engine='python', engine.path='C:/Python27/ArcGISx6410.4/python.exe', eval=F}
import arcpy
import os
from arcpy.sa import *
...
# NLCD2011
wetlands_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/AllWetlands'
wetrpu_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/AllWetlands_rpu'
isolated_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/IsolatedWetlands'
paths_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/CostPaths'
wetpoints_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPoints'
...
1. Read in duplicated paths values from chunk above along with the cost path rasters
2. Create numpy arrays from both, flatten 2d array to 1d array, find duplicated values and replace (Ryan can elaborate)
```{r, engine='python', engine.path='C:/Python27/ArcGISx6410.4/python.exe', eval=F}
import os
import arcpy
from arcpy.sa import *
...
wetpt_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD'+year+'/WetlandPoints/'
cost_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/CostPaths/'
...
isolated_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/IsolatedWetlands'
paths_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/CostPaths'
frmto_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/FlowTables'
...
year = '2001'
numpy_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/WetPaths_npy/'
frmto_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/FlowTables/'
Paths_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year + '/WetlandPath/CostPaths/'
...
year = '2001'
path_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year +'/FullStreams'
out_dir = 'L:/Priv/CORFiles/Geospatial_Library_Projects/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD' + year +'/FlowLengthsDown/'
