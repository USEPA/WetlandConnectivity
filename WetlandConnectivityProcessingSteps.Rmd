...
for (i in 1:length(path_list)){
  ...
  rds_file = paste0(fesri_dir, 'qa_', substr(path_list[i], 14, 16), '.rds')
  wetpoint = readRDS(rds_file)[, c(1:5)]
  ...
  print(nrow(tmp))
  write.csv(tmp, file = paste0(path_dir, '/duplicated_', substr(path_list[i], 14, 16),'.csv'), row.names=F)
}
```
...
for in_strlink in files: 
    ...
    #        np.place(rst, query1, sequence) #This code kept crashing python
        ...
        arcpy.env.mask = cost
        arcpy.env.snapRaster = cost
        arcpy.CopyRaster_management(tempraster, outRas, "", "", "", "", "", "32_BIT_UNSIGNED")
