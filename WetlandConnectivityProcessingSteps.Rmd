...
def write_raster(raster, path, xsize, ysize, nodata):
    format = "GTiff"
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                if not arcpy.Exists(wetpoints_dir + "/WetlandPoints" + dirs[-3:]+ ".tif"):
                    ...
                    fac = read_raster(nhddir +"/NHDPlus" +region + "/NHDPlus" + hydro + "/NHDPlusFdrFac"  + dirs[-3:] + "/fac")
                    xsize = fac[0]; ysize = fac[1]
                    fac = fac[2]
                    wet = read_raster(wetrpu_dir + "/WetlandsRgnGrp_" + dirs[-3:] + ".tif")[2]
                    ...
                    empty.flat[locs] = unq
                    #write out raster
                    write_raster(empty, wetpoints_dir + "/WetlandPoints" + dirs[-3:] + ".tif", xsize, ysize, mn)
                    del(empty); del(unq)
```  
