    ...
    for hydro in inputs[region]:
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                ...
                if not arcpy.Exists(outCostPath):
                ...
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLink" + rpu + ".tif"):
                    ...
                    outCostPath = Raster(paths_dir + "/CostPaths/CostPath" + rpu + ".tif")
                ...
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLink" + rpu + ".shp"):
                    ...
                    arcpy.RasterToPolyline_conversion(in_raster=working_dir + "/ScratchDir/StreamLink" + rpu + ".tif", out_polyline_features=working_dir + "/ScratchDir/StreamLink" + rpu + ".shp", background_value="NODATA", minimum_dangle_length="0", simplify="NO_SIMPLIFY", raster_field="VALUE")
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".shp"):
                    # Split the polylines at wetland points
                    arcpy.SplitLineAtPoint_management(in_features=working_dir + "/ScratchDir/StreamLink" + rpu + ".shp", point_features = wetpoints_dir + "/WetlandPoints" + rpu+ ".shp", out_feature_class = working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".shp", search_radius="90 Meters")
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".tif"):
                    # Now convert polylines back to a raster
                    arcpy.PolylineToRaster_conversion(in_features = working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".shp", value_field="ARCID", out_rasterdataset=working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".tif", cell_assignment="MAXIMUM_LENGTH", priority_field="NONE", cellsize="30")
                                   
                EucAlloc = EucAllocation(working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".tif", "30", "", "30", "Value", "", "")
...
#                    EucAlloc = Raster(temp + '/StreamLink_EucAlloc.tif')
                ...
                if not arcpy.Exists(paths_dir + "/CostPaths/StreamLink" + rpu + ".tif"): 
                    Adjusted.save(paths_dir + "/CostPaths/StreamLink" + rpu + ".tif")
```
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                ...
                if not arcpy.Exists(working_dir + "/StreamLink" + rpu + ".tif"):
                    ...
                    # Execute StreamLink
                    outStreamLink = StreamLink(outCostPath, fdr)
                    outStreamLink = SetNull(outStreamLink, outStreamLink, "VALUE <= 0")
                    # Save the output
                    if not arcpy.Exists(working_dir + "/ScratchDir/StreamLink" + rpu + ".tif"):
                        outStreamLink.save(working_dir + "/ScratchDir/StreamLink" + rpu + ".tif")
                    ...
                    if not arcpy.Exists(working_dir + "/ScratchDir/StreamLinkSplit.tif"):
                        arcpy.PolylineToRaster_conversion(in_features = working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".shp", value_field="FID", out_rasterdataset=working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".tif", cell_assignment="MAXIMUM_LENGTH", priority_field="NONE", cellsize="30")
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                ...
                if not os.path.exists(outDbf):
                    ...
                    shift1 = arcpy.Shift_management(Paths, "shift1.tif", "-30", "0", Paths)
                    shift2 = arcpy.Shift_management(Paths, "shift2.tif", "-30", "30", Paths)
                    shift4 = arcpy.Shift_management(Paths, "shift4.tif", "0", "30", Paths)
                    shift8 = arcpy.Shift_management(Paths, "shift8.tif", "30", "30", Paths)
                    shift16 = arcpy.Shift_management(Paths, "shift16.tif", "30", "0", Paths)
                    shift32 = arcpy.Shift_management(Paths, "shift32.tif", "30", "-30", Paths)
                    shift64 = arcpy.Shift_management(Paths, "shift64.tif", "0", "-30", Paths)
                    shift128 = arcpy.Shift_management(Paths, "shift128.tif", "-30", "-30", Paths)                   
...
ctl_path = 'J:/GitProjects/Wetland Connectivity/'
...
path_dir = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/CostPaths'
points_dir = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPoints'
wetlands_dir = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/AllWetlands_rpu'
fromto_dir = 'L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/FlowTables'

path_list = list.files(path_dir)
path_list = variables[grep("StreamLink", path_list)]
path_list = path_list[grep('\\.tif$',path_list)]

point_list = list.files(points_dir)
point_list = point_list[grep("WetlandPoints", point_list)]
point_list = point_list[grep('\\.shp$',point_list)]

wetland_list = list.files(wetlands_dir)
wetland_list = wetland_list[grep("Wetlands", wetland_list)]
wetland_list = wetland_list[grep('\\.dbf$',wetland_list)]

fromto_list = list.files(fromto_dir)
fromto_list = fromto_list[grep("FrmTo", fromto_list)]
fromto_list  = fromto_list[grep('\\.tif$',fromto_list)]

fromtotable_list = list.files(fromto_dir)
fromtotable_list = fromtotable_list[grep("FrmTo", fromtotable_list)]
...
count_nopaths=0
for (i in 1:59){
  streamras = raster(paste0(path_dir,'/',path_list[i]))
  ...
  WetlandsWithPath = wetpoint[,1:2]
  names(WetlandsWithPath)[1] = 'WET_ID'
  WetlandsNoPath = as.data.frame(wetland[!wetland$WET_ID %in% wetpoint$WET_ID,])
  AllWetlands = merge(wetland, WetlandsWithPath@data, by='WET_ID', all.x=T)
  ...
  write.csv(AllWetlands, paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/LookupTables/AllWetlands_StreamLink_Lookup_',str_sub(strsplit(wetland_list[i],'\\.')[[1]][1],-3),'.csv'))
  write.csv(WetlandsWithPath, paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/LookupTables/WetlandsWithPath_StreamLink_Lookup_',str_sub(strsplit(wetland_list[i],'\\.')[[1]][1],-3),'.csv'))
  write.csv(WetlandsNoPath, paste0('L:/Priv/CORFiles/Geospatial_Library/Data/Project/WetlandConnectivity/SpatialDataInputs/Wetlands_NLCD2011/WetlandPath/LookupTables/WetlandsNoPath_StreamLink_Lookup_',str_sub(strsplit(wetland_list[i],'\\.')[[1]][1],-3),'.csv'))
}
```
...
ctl_path = 'D:/WorkFolder/WetConnect_Aug2016/WetlandScripts/'
ctl = pd.read_csv(ctl_path + 'ControlTable_WetlandCatments.csv')
