    ...
    for hydro in inputs[region]:
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                ...
                Fullstreamsnull = fullstreams_dir + "/FullStreamsFDRNull" + dirs[-3:] + ".tif"
                if not arcpy.Exists(outCostPath):
                    outCostPath = CostPath(WetPoints, nhddir + "/NHDPlus" + region + "/NHDPlus" + hydro + "/NHDPlusHydrodem" + dirs[-3:] + "/hydrodem",Fullstreamsnull, "EACH_CELL", "GRID_CODE")
                ...
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLink" + dirs[-3] + ".shp"):
                ...
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLinkSplit" + dirs[-3] + ".shp"):
                ...
                if not arcpy.Exists(working_dir + "/ScratchDir/StreamLinkSplit" + dirs[-3] + ".tif"):
                    # Now convert polylines back to a raster
                    arcpy.PolylineToRaster_conversion(in_features = working_dir + "/ScratchDir/StreamLinkSplit" + rpu + ".shp", value_field="OBJECTID", out_rasterdataset=working_dir + "/ScratchDir/StreamLinkSplit" + dirs[-3] + ".tif", cell_assignment="MAXIMUM_LENGTH", priority_field="NONE", cellsize="30")
