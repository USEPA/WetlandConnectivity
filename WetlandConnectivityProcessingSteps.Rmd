    ...
    for hydro in inputs[region]:
        ...
        for dirs in os.listdir(nhddir + "/NHDPlus%s/NHDPlus%s"%(region, hydro)):
            if dirs.count("FdrFac") and not dirs.count('.txt') and not dirs.count('.7z'):
                ...
                wetland_arr = np.array(wetland_arr.GetRasterBand(1).ReadAsArray())                
                ...
                newraster = array2raster(isolated_dir + '/isoWetlands_' + rpu + '.tif', wetland_ras, z)
                newRaster = Raster(isolated_dir + '/isoWetlands_' + rpu + '.tif')
