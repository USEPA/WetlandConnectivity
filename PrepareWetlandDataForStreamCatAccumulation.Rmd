---
title: "Prep Wetland cat Data for Accumulation"
output: 
  html_document: 
    theme: yeti
---
...
sc_dir <- 'L:/Priv/CORFiles/Geospatial_Library_Projects/NRSA/NRSA0809/SplitCats_tifs/'
...
sites <- as.character(ndata$SITE_ID)
for(i in 1:length(sites)){
  print(sites[i])
  sc <- paste0(sc_dir, 'ws', sites[i], '.tif')
  if(file.exists(sc)){
    sc <- raster(sc)
    sc[!is.na(sc)] <- 1
    ...
    if(i == 1){
      outdf <- data.frame(SITE_ID = sites[i], SUM=SUM, COUNT=COUNT)  
    }else{
      outdf <- rbind(outdf, 
                     data.frame(SITE_ID = sites[i], SUM=SUM, COUNT=COUNT))
    }
  }
}
  ...
  for(i in 1:length(sites)){
    ...
    if(file.exists(sc)){
      ...
      tmp <- data.frame(WET_ID=NA, WtCatCount=0, SUM=0, 
                        SITE_ID=sites[i], CatCount=cellStats(sc, sum), 
...
# Summarize wetland N for each SITE_ID and by wetland type for each SITE_ID
...
# Combine StreamCat wetland accumulations to single (2.6 million records) files 
...
TNs <- c('TN_TotalKg_','TN_ByAreaTime_','TN_ByTraveTime_','TN_ByWtArea_')
hydro.rgns <- c("01","02","03S","03N","03W","04","05","06","07","08","09","10L","10U","11","12","13","14","15","16","17","18")
...
for(i in 1:length(files)){
  ...
  saveRDS(outdf, paste0(nmodel_dir, 'splitcat-metrics/',files[i],'WsMetrics.rds'))
}
...
for(i in 1:length(Types)){
  ...
  tmp <- tmp[, c('SITE_ID','CatCount','SUM')]
  nms <- paste0(Types[i], c('CatCount','CatSum'))
...
nout[is.na(nout)] <- 0
nout <- nout[ , -grep('Count',names(nout))]
