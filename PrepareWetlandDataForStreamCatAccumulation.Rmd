...
## Combine accumulated wetland **catchment** areas by wetland type
...
for(i in 1:length(hydro.rgns)){
  ...
  for(k in 1:length(Types)){
    print(Types[k]) 
    tmp <- read.csv(paste0(accum_dir, 'WtCatAreasByType_', Types[k], '_', hydro.rgns[i], '.csv'))[, c('COMID','UpCatCount')]
...
upwtcat <- readRDS(paste0(nmodel_dir, 'splitcat-metrics/TN_TotalKg_UpCatMetrics.rds'))
upwtcat <- subset(upwtcat, select = c(COMID, UpWtCatCount))
...
nout <- merge(nout, upwtcatbytype, by='COMID')
...
#Prep N input data
sums <- grep('Sum', names(ninput))
ninput$WtKgNWs <- rowSums(ninput[sums])
ninput$WsAreaHa <- (ninput$UpCatAreaSqKm + ((ninput$CatCount * 900) / 1e6)) * 100
...
ninput$WtCatAreaWsHa <- (rowSums(ninput[grep('WtCatCount', names(ninput))]) * 900) / 10000
ninput$testsum <- (rowSums(ninput[grep('WtCatCount', names(ninput))]) 

ninput <- merge(ninput, fullWsN, by='SITE_ID')
ninput <- subset(ninput, select = c(SITE_ID, COMID, WtKgNWs, AllKgNWs, WsAreaHa, DSlpWtWs, DSlpWtNrWs, RiparWtAreaHa, NRSurWtAreaHa, NRSubPdWtAreaHa, NRSubWdWtAreaHa))
ndata <- merge(nout, ninput, by='SITE_ID')
ndata$NwKgNWs <- ndata$AllKgNWs - ndata$WtKgNWs
ndata$q.ann_ha <- ndata$q.ann / ndata$WsAreaHa

write.csv(ndata, paste0(nmodel_dir, 'splitcat-metrics/fullwatershed-splitcat-wetland-metrics-BYWETLAND-TYPE.csv'), row.names = F)
